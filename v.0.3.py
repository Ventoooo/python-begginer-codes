...
while True:
    ...
    #   função para 3
        ...
        with open(str(data_()) + '.txt', 'w') as historico:
            ...
            for i, c in enumerate(lista_pickle['lucros']):
                a = f'{i + 1}-    {c["Descrição"]} ===> {c["Valor"]:.2f}R$ as {c["Horario"]}Hs. \n'
                historico.write(str(a))
            historico.write("\nGASTOS:\n\n")
            for i, c in enumerate(lista_pickle['gastos']):
                b = f'{i + 1}-    {c["Descrição"]} ===> {c["Valor"]:.2f}R$ as {c["Horario"]}Hs. \n'
    ...
    0- \033[31mGasto\033[m
    1- \033[32mlucro\033[m''')
        del_way = str(input(': '))
        if del_way == '0':
            while True:
